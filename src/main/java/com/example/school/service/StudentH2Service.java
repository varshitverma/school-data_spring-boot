...
public class StudentH2Service implements StudentRepository {
    ...
    public ArrayList<Student> getStudents() {
        Collection<Student> studentColletion = db.query("select * from student", new StudentRowMapper());
        ArrayList<Student> students = new ArrayList<>(studentColletion);
        return students;
    }
    ...
    public String addStudents(ArrayList<Student> students) {
        String bulkCount = String.valueOf(students.size());
        for (Student student : students) {
            db.update("insert into student(studentName, gender, standard) values(?, ?, ?)",
            student.getStudentName(), student.getGender(), student.getStandard());
        }
        String messageStr = String.format("Successfully added %s students", bulkCount);
        return messageStr;
    }
